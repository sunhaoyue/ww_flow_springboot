             ...
             <tr>
               ...
               <script type="text/javascript">
					...
					$(document).ready(function() {
						$("#uploadify").uploadify({
							'uploader'       : '<%=request.getContextPath()%>/js/upload/uploadify.swf',
							'script'         : '<%=request.getContextPath()%>/common/upload.action',
							'buttonImg'		 : '<%=request.getContextPath()%>/js/upload/browse.png',
							'width'			 : 78,
							'height'		 : 25,
							'cancelImg'      : '<%=request.getContextPath()%>/js/upload/cancel.png',
							'queueID'        : 'fileQueue',
							'auto'           : false,
							'multi'          : true,
							'buttonText'	 : 'BROWSE',
						...
						if(rowIds){
						    for(var i = 0, j = rowIds.length; i < j; i++) {
								...
								if (curRowData['id'] == attachmentId) {
									...
									$.ajax({
									    type: "POST",
									    url: '<%=request.getContextPath()%>/common/uploadDeleteFile.action',
									    dataType:'json',
					...
					function downloadAttachmentFile(attachmentId){
						window.location.href='<%=request.getContextPath()%>/common/download.action?id='+attachmentId;
					}
					...
					function renderOperation(cellvalue, options, rowObject) {
						...
						if(rowObject.isNew || template || renew){
							btns.push("<input type='button' value='下载' onclick='downloadAttachmentFile("+rowObject.id+")'>");
							btns.push("<input type='button' value='删除' onclick='removeOneRow("+rowObject.id+")'>");
						}
						return btns.join('');
					}
			   </script>
               ...
               <script>
			 		jQuery("#attachmentlist").jqGrid({ 
			 			datatype: "local", 
			 			height: 100,
			 			editurl: '<%=request.getContextPath()%>/common/upload.action', // this is dummy existing url
			 			loadonce: true,
			 			...
			 			colModel:[ {name:'attachmentName',index:'attachmentName', width:200, formatter:makeURL}, 
			 				 ...
			 				 {name:'isNew',index:'isNew', width:150, hidden:true},
