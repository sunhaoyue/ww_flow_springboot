 ...
 <div class="rightpage01">
  <div class="headbg01">
    <table width="100" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="35"><img src="<%=request.getContextPath()%>/images/ico09.gif" width="33" height="27" /></td>
        <td><a href="#" id="confirm">确定</a></td>
      </tr>
    </table>
   </div>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><table width="400" border="0" cellspacing="8" cellpadding="0">
        <tr>
          ...
          <td><select name="branch" id="branch" onchange="selectBranch(this)">
        ...
        <script>  
			   ...
			   document.getElementById("endTime").value = nextMonthDate; 
		</script>
        <tr>
          <td width="100" align="right">表单状态：</td>
          <td><select name="formStatus" id="formStatus">
        ...
        <tr id="Title" style="display:none">
          ...
          <td><input type="text" name="title" id="title" class="width01"/></td>
        </tr>
        <tr id="FormNum" style="display:none">
          <td align="right">表单编号:</td>
          <td><input type="text" name="formNum" id="formNum" class="width01"/></td>
        </tr>
      </table></td>
    </tr>
  </table>
		...
		function renderFlowType(cellvalue, options, rowObject){
 		...
 		$(function(){ 
			  $("#list1").jqGrid({
			    ...
			    caption: '参考内联列表',//表格标题
			    url:'<%=request.getContextPath()%>/management/searchMyWorks.action',//请求路径
			    ...
			    rowList:[10,20,30],//下拉选择页面大小
 				...
 				colModel :[ //列模型 
			      ...
			      {name:'flowTypeName', index:'flowTypeName',width:60,formatter:renderFlowType},
			      {name:'title', index:'title',width:90},			      
			      {name:'creatorName', index:'creatorName',width:90},
			      {name:'actualName', index:'actualName', width:90}, 
