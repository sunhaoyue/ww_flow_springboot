...
<script type="text/javascript" src='${ctx }/js/layout.js'></script>
	<script type="text/javascript">
			...
			function assignWork(personDetail){
				...
				document.getElementById('applyForm').action="${ctx}/nneilian/neiLianEditAction!assignWork.action";
			...
			function adminAssignWork(personDetail){
				...
				var assignId = personDetail.deptId+","+ personDetail.employeeId+","+personDetail.postCode;		
				$("#assignId").val(assignId);
				document.getElementById('applyForm').action="${ctx}/nneilian/neiLianEditAction!adminAssignWork.action";
				document.getElementById('applyForm').submit();
			}
			...
			function reject(){
				...
				$.dialog.tips('数据正在提交中，请稍候...', 6000, 'tips.gif');
				document.getElementById('applyForm').action="${ctx}/nneilian/neiLianEditAction!rejectWork.action";
				document.getElementById('applyForm').submit();				
			}
			...
			function renew(){
				...
				document.getElementById('applyForm').action="${ctx}/nneilian/neiLianEditAction!renewWork.action?type=renew&workId="+workId;
				document.getElementById('applyForm').submit();				
			}
			...
			function applySubmit(){
				...
				$.post("${ctx}/nneilian/neiLianEditAction!checkMgrPerson.action",postData,function(data){
					if(data.status == "error"){
						alert(data.message);
						return;
					}
					$.dialog.tips('数据正在提交中，请稍候...', 6000, 'tips.gif');
					document.getElementById("applyForm").action="${ctx}/nneilian/neiLianEditAction!confirmWork.action";				
					document.getElementById('applyForm').submit();
				},"json");
			}
	  	...
	  	<tr>
	  		<s:if test="#request.workStage!=null&&#request.role!='ASSIST_ASSIGNED'&&#request.role!='ASSIST_NOT_ASSIGNED'">
		  	    <s:if test="#request.workStage!='COPYDEPT'&&#request.workStage!='REJECT'">
				   ...
				   <s:elseif test="#request.workStage=='JOINTSIGN'||#request.workStage=='INNERJOINTSIGN'||#request.workStage=='CENTERJOINTSIGN'||#request.workStage=='XZJOINTSIGN'">
			...
			<s:if test="(#request.workStage=='JOINTSIGN'||#request.workStage=='CENTERJOINTSIGN'||#request.workStage=='XZJOINTSIGN'||#request.workStage=='BOSS_SIGN')&&#request.role!='ASSIST_NOT_ASSIGNED'&&#request.hbMgrCanSign">
			...
			<s:if test="#request.workStage=='REJECT' && !#request.assist">
	         		<td><input type="button" class="button blue medium" value="本人确认" onclick="cancel()"/></td>
					...
					--%>
			         ...
			         <s:if test="#request.workStage=='REJECT'">
			         	<td align="right" valign="middle"><input id="renewButton" type="button" class="button blue medium" value="重新起案" onclick="renew()"></td>
			         </s:if>
			...
			<td><input type="button" class="button blue medium" value="打印" id="printForm" onclick="printForm('<s:property value="flow.formNum"/>', 'nl')" /></td>
...
if(!workStage){
	...
	if(workStage=='INNERJOINTSIGN' || workStage == 'JOINTSIGN' || workStage == 'CENTERJOINTSIGN' || workStage == 'XZJOINTSIGN' || workStage=='CENTER_CHENGHE' || workStage == 'BOSS_SIGN' || workStage == 'CONFIRM'){
		 $("#query3").attr("style", "display:none;"); //2016-10-26
		//document.getElementById("query3").disabled = "disabled";
