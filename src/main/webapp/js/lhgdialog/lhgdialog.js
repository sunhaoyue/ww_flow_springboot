...
lhgdialog.setting =
{
	...
	button: null,				// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数，如果为false那么不显示关闭按钮 important！！！！！！！！！！！！！！！！！！
...
lhgdialog.alert = function( title,content,icon, callback, parent )
{	
	title=title ||  '提示';
	return lhgdialog({
		title:title,
...
lhgdialog.confirm = function( title,content, yes, no,cancel,parent )
{	
	title=title || "确认";
	var configBtn = [];
	if(cancel){
...
lhgdialog.prompt = function( title , content, yes, multiline, value, parent )
{
	title=title || "提示";
	value = value || '';
	var input;
	...
	if(multiline){
		contentDiv = [
		   '<div style="margin-bottom:5px;font-size:12px">',
			...
			'</div>',
			'<div>',
				'<textarea " style="width:240px;padding:6px 4px;height:30px;" >',
					value,
				'</textarea>',
			'</div>'
		].join('');
		...
		contentDiv = [
			...
			'<div>',
				'<input value="',
					value,
				'" style="width:240px;padding:6px 4px" />',
			'</div>'
		].join('');		
	}
...
lhgdialog.progressbarDlg = function(title,msg)
{	
	var contentDiv=[];
		contentDiv = [
	   '<div style="margin-bottom:5px;font-size:12px">',
	...
	return lhgdialog({
		...
		max:false,
		min:false,
		cancel:false,//当为false的时候隐藏关闭按钮
		content:contentDiv,
		parent: parent || null,
		init:function(){
			var temp = this.DOM.content[0].getElementsByTagName('div')[1];
			//$(temp).progressBar({interval:300,increment:5,width:300,showPercent:false,text:'Loading...'});
			var $ds = $('<span>asdfjasidjfoasidjop</span>');
			$ds.addClass("mytest");
			$ds.appendTo(temp);
		}
	});
